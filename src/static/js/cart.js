...
try {
    ...
    console.log("Holy Smoky! There is an error in cart.js file. Error: " + error + ". Please check the file." );
}
...
function updateUserOrderDatabase(productId) {
    if (user == 'AnonymousUser') {
        ...
        if (cart[productId] == undefined){
            cart[productId] = {'quantity': cart_quantity}
            toastr.success('Add to Cart!')
        }else{
            var quantity = cart[productId]['quantity'] 
            cart[productId]['quantity'] = parseInt(quantity)+parseInt(cart_quantity)
